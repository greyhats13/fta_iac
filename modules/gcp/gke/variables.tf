...
variable "node_config" {
  ...
  type = map(object({
    ...
    machine_type    = string  # The machine type to use for the node pool e.g. e2-medium, n1-standard-2, etc.
    disk_size_gb    = number       # The size of the disk attached to each node e.g. 20.
    disk_type       = string       # The type of disk attached to each node e.g. pd-standard, pd-ssd.
